...
if (!isset($_FILES['uploadfilensn']) || $_FILES['uploadfilensn']['error'] !== UPLOAD_ERR_OK) {
    die("Gagal upload file.");
}

// Simpan sementara file yang diupload
$tmpFilePath = $_FILES['uploadfilensn']['tmp_name'];
...
for ($i = 1; $i < count($data); $i++) {
    ...
    $NSN      = $koneksi->real_escape_string(trim($row[2]));
    ...
    $Date_last_change = $koneksi->real_escape_string(trim($row[13]));
    ...
    $ShelfLifeCd_SLC = $koneksi->real_escape_string(trim($row[36]));
    ...
    $sql = "INSERT INTO nsn45 (
    NIIN, NSC, NSN, RPDMRC, `NIIN SC`, INC, Type, FIIG, FMSN, Item_Name,
    Country, Date_NIIN, Date_of_NIIN_Assignment, Date_last_change, NIIN_Type,
    ...
    CIIC, `ShelfLifeCd_(SLC)`, Date_Last_Change2, UoM_Rel_NSN, Project_Shortname,
    Project_Longname, CPV, CPV_Text, RNAAC_ISO, International_Registered_Users_ISO) 
    VALUES (
    '$NIIN', '$NSC', '$NSN', '$RPDMRC', '$NIIN_SC', '$INC', '$Type', '$FIIG', '$FMSN', '$Item_Name',
    '$Country', '$Date_NIIN', '$Date_of_NIIN_Assignment', '$Date_last_change', '$NIIN_Type',
    '$NCAGE', '$Manufacturer_Name', '$Reference', '$RNFC', '$RNCC',
    '$RNVC', '$DAC', '$RNAAC', '$RNSC', '$RNJC',
    '$Has_Doc', '$International_Registered_Users', '$National_registered_users', '$Repl_NIIN_1', '$Repl_NIIN_2',
    '$Segment_M', '$MOEC', '$Unit_of_Issue_Code', '$UnitIss_Conv_Factor', '$Form_Unit_Issue',
    '$CIIC', '$ShelfLifeCd_SLC', '$Date_Last_Change2', '$UoM_Rel_NSN', '$Project_Shortname',
    '$Project_Longname', '$CPV', '$CPV_Text', '$RNAAC_ISO', '$International_Registered_Users_ISO')";
